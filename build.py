...
function setupStaticListeners() {
  ...
  function handleChartTypeChange(newType) {
    ...
    if (isBeta) {
      ...
      // Hide external and structural logger checkboxes; only show room loggers
      const _m = dataset().meta;
      const _roomSet = new Set(_m.roomLoggers || []);
      const loggerDiv = document.getElementById('logger-checkboxes');
      // Use data attributes on checkboxes to determine which to show
      ...
      for (const child of Array.from(loggerDiv.children)) {
        if (child.classList && child.classList.contains('sub-section-title')) {
          // Check if next checkbox siblings are room loggers
          const nextCb = child.nextElementSibling;
          let sectionIsRoom = false;
          // Look ahead to find first checkbox in this section
          ...
          while (sib && !sib.classList.contains('sub-section-title')) {
            ...
            if (cb) { sectionIsRoom = _roomSet.has(cb.dataset.loggerId); break; }
            sib = sib.nextElementSibling;
          }
          inRoomSection = sectionIsRoom;
          child.style.display = inRoomSection ? '' : 'none';
        } else {
          child.style.display = inRoomSection ? '' : 'none';
        }
      }
...
function renderBetaLag() {
  ...
  const yTop = Math.ceil((maxLag * 1.3 + 0.5) * 2) / 2; // round up to nearest 0.5, with padding for text labels
  return {
    traces, title,
    layout: {
      autosize: true, font: {family: 'Ubuntu, sans-serif'},
...
function renderBetaQuality() {
  ...
  for (const loggerId of m.loggers) {
    ...
    if (temp.length > OUTLIER_WINDOW * 2) {
      const outlierX = [], outlierY = [], outlierText = [];
      for (let i = OUTLIER_WINDOW; i < temp.length - OUTLIER_WINDOW; i++) {
        ...
        if (std > 0 && temp[i] != null && Math.abs(temp[i] - mean) > 3 * std) {
          ...
          outlierY.push(y);
          const dev = ((temp[i] - mean) / std).toFixed(1);
          outlierText.push(`${nameWithSrc}<br>Suspect reading: ${temp[i].toFixed(1)}\u00b0C<br>Local mean: ${mean.toFixed(1)}\u00b0C (\u00b1${std.toFixed(1)})<br>Deviation: ${dev}\u03c3<br>%{x}`);
        }
      }
      if (outlierX.length > 0) {
        traces.push({
          ...
          showlegend: false,
          hovertemplate: outlierText.map(t => t + '<extra></extra>'),
        });
      }
    }
    ...
    if (anomRanges[loggerId]) {
      ...
      const reason = rng.reason || 'Flagged as anomalous by admin';
