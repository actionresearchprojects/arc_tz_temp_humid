...
#chart-type option[value="beta"] { color: #c0392b; font-weight: 700; }
#beta-chart-type { color: #c0392b; font-weight: 600; border-color: #c0392b; }
      ...
      <div id="time-bar-top">
        <div id="time-bar-left">
          ...
          <select id="chart-type">
            ...
            <option value="beta" style="color:#c0392b;font-weight:700" data-i18n="betaFeatures">Beta Features</option>
          </select>
          <select id="beta-chart-type" style="display:none;color:#c0392b;font-weight:600;border-color:#c0392b">
...
function setupStaticListeners() {
  ...
  function handleChartTypeChange(newType) {
    ...
    if (isBeta) {
      ...
      // Hide external and structural sections; only show room loggers
      const _m = dataset().meta;
      const _extSet = new Set(_m.externalLoggers || []);
      const _roomSet = new Set(_m.roomLoggers || []);
      const loggerDiv = document.getElementById('logger-checkboxes');
      // Walk through sub-section-title elements and hide external/structural sections
      let currentSection = null;
      for (const child of loggerDiv.children) {
        if (child.classList && child.classList.contains('sub-section-title')) {
          currentSection = child.textContent.trim();
          const isRoom = currentSection === t('sectionRoom');
          child.style.display = isRoom ? '' : 'none';
    ...
    if (iter.selectedCrossLoggers) {
      for (const otherKey of Object.keys(iter.selectedCrossLoggers)) {
        ...
        for (const lid of crossSet) {
          ...
          const color = otherM.colors[lid] || '#999';
          ...
          for (const metric of ['temperature','humidity']) {
            ...
            traces.push({x, y, type:'scatter', mode:'lines',
              ...
              line:{color, width:1.4, dash:'dot'}, opacity:0.5, connectgaps:false,
...
function renderBetaDecrement() {
  ...
  return {
    ...
    layout: {
      ...
      yaxis: {title: t('betaDecrementAxis'), showgrid: true, gridcolor: '#eee', range: [0, Math.max(1.2, ...(loggerDecrement.length ? loggerDecrement : [0])) * 1.15], zeroline: true},
...
function renderBetaLag() {
  ...
  const maxLag = loggerLag.length > 0 ? Math.max(...loggerLag) : 8;
  return {
    traces, title,
    layout: {
      autosize: true, font: {family: 'Ubuntu, sans-serif'},
      margin: {l: sm?45:65, r: sm?8:20, t: sm?30:40, b: sm?80:100},
      xaxis: {title: 'Room Logger <span style="color:#aaa">(source type)</span>', tickangle: -30, automargin: true},
      yaxis: {title: t('betaLagAxis'), showgrid: true, gridcolor: '#eee', range: [0, Math.ceil(Math.max(8, maxLag * 1.2))], fixedrange: true},
      plot_bgcolor: 'white', paper_bgcolor: 'white',
      hovermode: 'closest',
    },
    _noData: !hasData,
    ...
    if (temp.length > OUTLIER_WINDOW * 2) {
      ...
      if (outlierX.length > 0) {
        traces.push({
          ...
          showlegend: false,
          hovertemplate: `${nameWithSrc}<br>Suspect reading<br>%{x}<extra></extra>`,
        });
      }
    }
