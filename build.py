...
function renderCompareSets() {
  ...
  for (let si = 0; si < state.compareSetCount; si++) {
    ...
    function mkCmpBtn(label, onClick) {
...
function renderAdaptiveComfort() {
  console.log('[COMFORT DEBUG] renderAdaptiveComfort called, compareEnabled=', state.compareEnabled);
  const {start, end} = getTimeRange();
  const m = dataset().meta;
  ...
  for (const iter of iterations) {
    ...
    for (const loggerId of (m.comfortLoggers || m.roomLoggers)) {
      ...
      // Legend-only trace (only in non-compare mode)
      if (!iter.setLabel) {
  ...
  console.log('[DENSITY DEBUG] showDensity=', state.showDensity, 'allExtTemps.length=', allExtTemps.length, 'compareEnabled=', state.compareEnabled);
  if (state.showDensity && allExtTemps.length > 30) {
    console.log('[DENSITY DEBUG] Adding density heatmap trace');
...
function _doRender() {
  console.log('[RENDER DEBUG] _doRender called, chartType=', state.chartType, 'compareEnabled=', state.compareEnabled);
