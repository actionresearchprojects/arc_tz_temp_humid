...
#chart .annotation { transition: opacity 0.2s !important; }
...
function positionComfortOverlays() {
  ...
  if (fl && fl.xaxis && fl.xaxis._offset !== undefined) {
    ...
    const axisCenterX = cr.left + fl.xaxis._offset + fl.xaxis._length / 2;
    // Title sits below the tick labels; use the full height minus a small offset from bottom
    const titleY = cr.top + cr.height - fl.margin.b / 3;
    rmIcon.style.display = '';
    // Place to the right of where the title text ends (estimate ~half title width from center)
    rmIcon.style.left = (axisCenterX + 140) + 'px';
    rmIcon.style.top = (titleY - 7) + 'px';
  } else {
