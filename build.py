...
<div id="main">
  ...
  <div id="chart-area">
    ...
    <div id="chart"></div>
    <button id="annotation-toggle" title="Hide details" style="display:none;position:absolute;z-index:60;width:18px;height:18px;padding:0;border:1px solid #ccc;border-radius:3px;background:rgba(255,255,255,0.85);color:#888;font-size:11px;line-height:18px;text-align:center;cursor:pointer;font-family:sans-serif;" onclick="toggleAnnotation()">&#x25BC;</button>
    <span class="info-i" id="rm-xaxis-info-icon" style="display:none;position:absolute;z-index:60;">i</span>
...
function positionComfortOverlays() {
  ...
  if (state.chartType !== 'comfort') { btn.style.display = 'none'; rmIcon.style.display = 'none'; return; }

  // Position annotation toggle next to the "Data ranges from..." annotation
  const annos = chartEl.querySelectorAll('.annotation');
  // Find the bottom-right annotation (dateRange annotation has paper xref/yref)
  let targetAnno = null;
  annos.forEach(a => {
    const r = a.getBoundingClientRect();
    // The dateRange annotation sits near the bottom-right of the plot
    if (r.bottom > chartRect.top + chartRect.height * 0.5) targetAnno = a;
  });
  if (targetAnno && !_annotationHidden) {
    const ar = targetAnno.getBoundingClientRect();
    btn.style.display = '';
    btn.style.left = (ar.left - chartRect.left - 22) + 'px';
    btn.style.top = (ar.top - chartRect.top + 1) + 'px';
  } else if (_annotationHidden) {
    // When hidden, put the restore button at bottom-right of plot area
    const fl = chartEl._fullLayout;
    if (fl) {
      const plotRight = fl.margin.l + fl.width;
      const plotBottom = fl.margin.t + fl.height;
      btn.style.display = '';
      btn.style.left = (plotRight - 22) + 'px';
      btn.style.top = (plotBottom - 22) + 'px';
    }
  } else {
...
(function() {
  ...
  items.forEach(({iconId, tipId, text, html}) => {
    const icon = document.getElementById(iconId);
    const tip  = document.getElementById(tipId);
    if (!icon || !tip) return;
    if (html) { tip.innerHTML = html; tip.style.pointerEvents = 'auto'; }
    ...
    if (html) {
      // Keep tip visible when hovering over it (so user can click links)
      let hideTimer;
