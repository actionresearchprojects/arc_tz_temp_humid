      ...
      <div id="time-bar-top">
        <div id="time-bar-left">
          ...
          <select id="chart-type">
            ...
            <option value="periodic">Average Profiles</option>
          </select>
...
function renderCompareSets() {
  ...
  for (let si = 0; si < state.compareSetCount; si++) {
    ...
    btnRow.style.cssText = 'display:flex;gap:4px;margin-bottom:3px;';
    const allBtn = document.createElement('button');
    allBtn.className = 'sel-btn';
    allBtn.textContent = 'All';
    allBtn.addEventListener('click', () => {
      loggerPool.forEach(id => { stateSet.add(id); });
      loggerWrap.querySelectorAll('input[data-cmp-logger]').forEach(cb => { cb.checked = true; });
      updatePlot();
    });
    ...
    noneBtn.addEventListener('click', () => {
      loggerPool.forEach(id => { stateSet.delete(id); });
      loggerWrap.querySelectorAll('input[data-cmp-logger]').forEach(cb => { cb.checked = false; });
      updatePlot();
    });
    ...
    // Add sections (external, room, structural)
    function addCmpSection(title, ids) {
      ...
      for (const id of ids) {
        ...
        lbl.innerHTML = '<input type="checkbox" data-cmp-logger="' + id + '" ' + (stateSet.has(id) ? 'checked' : '') + '> <span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:' + m.colors[id] + ';vertical-align:middle"></span> ' + m.loggerNames[id] + ttSuffix;
    ...
    for (const loggerId of m.loggers) {
      ...
      const freqLabel = state.historicMode
        ? (isOpenMeteo(loggerId) ? ' <span style="color:#aaa">(hourly avg.)</span>'
          ...
          : '') : '';
      const lgGroup = iter.setLabel ? loggerId + '_s' + iter.setIndex : loggerId;
      ...
      for (const metric of ['temperature','humidity']) {
        ...
        traces.push({x, y, type:'scatter', mode:'lines', name: name + meteoSuffix(loggerId) + omniSuffix(source) + freqLabel, line:{color, width:1.4},
          opacity:0.35, connectgaps:false, legendgroup:lgGroup, showlegend:firstMetric, meta:{loggerId},
...
function extSourcesForPoint(spans, tsMs, m) {
  ...
  // The running mean at this point is influenced by the preceding ~7 days.
  // Find all sources that contributed daily temps in that window.
  const DAY_MS = 86400000;
  const pointDate = toEATString(tsMs).slice(0, 10);
  const windowStart = toEATString(tsMs - 7 * DAY_MS).slice(0, 10);
  const srcLabel = (id) => {
  ...
  for (const sp of spans) {
    // Does this span overlap with [windowStart, pointDate]?
    if (sp.to >= windowStart && sp.from <= pointDate && !seen.has(sp.source)) {
      seen.add(sp.source);
      sources.push(srcLabel(sp.source));
    }
  }
  return sources.join(' + ') || '';
}
  ...
  for (const iter of iterations) {
    ...
    for (const loggerId of (m.comfortLoggers || m.roomLoggers)) {
      ...
      traces.push({x:filtered.extTemp, y:filtered.temperature, type:'scatter', mode:'markers',
        ...
        hovertemplate:`${namePrefix + m.loggerNames[loggerId]}<br>Running mean: %{x:.1f}°C<br>Room temp: %{y:.1f}°C<br>Ext. source: %{customdata}<br>Sensor: ${cSource}${cIdLabel}<extra></extra>`});
    ...
    for (const loggerId of m.loggers) {
      ...
      for (const metric of ['temperature', 'humidity']) {
        ...
        const trace = {
          ...
          legendgroup: lgGroup, showlegend: firstMetric,
          meta: {loggerId},
