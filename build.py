...
#chart .annotation { transition: opacity 0.2s; }
#chart .annotation rect { pointer-events: all; cursor: default; }
#chart .annotation:hover { opacity: 0.08; }
...
function positionComfortOverlays() {
  ...
  // Try multiple selectors for x-axis title (Plotly versions vary)
  const xTitleEl = chartEl.querySelector('.g-xtitle') || chartEl.querySelector('.xtitle') || chartEl.querySelector('[data-unformatted]');
  if (xTitleEl) {
    ...
    // Fallback: position based on chart layout
    const fl = chartEl._fullLayout;
    if (fl) {
      const cr = chartEl.getBoundingClientRect();
      const plotCenterX = cr.left + fl.margin.l + fl.width / 2;
      const plotBottom = cr.top + fl.margin.t + fl.height;
      // x-axis title sits in the bottom margin, roughly 30px below plot
      rmIcon.style.display = '';
      rmIcon.style.left = (plotCenterX + 120) + 'px';
      rmIcon.style.top = (plotBottom + 18) + 'px';
    } else {
      rmIcon.style.display = 'none';
    }
  }
